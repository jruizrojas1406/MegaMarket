(function init($, window, undefined) {
    ...
    var template = function () {
        ...
        // modal parts
        this.modal = $('<div class="modal fade" tabindex="-1" role="dialog"><div class="modal-dialog"><div class="modal-content"></div></div></div>');
        ...
        this.init = function () {
            ...
            this.header.appendTo(this.modalContent);
            this.content.appendTo(this.modalContent);
            this.footer.appendTo(this.modalContent);
        }
    ...
    dialog.show = function (options) {
        ...
        mdiag.modal.on('hidden.bs.modal', function (e) {
    ...
    /* Eventos Adicionados Anderson
    ---------------------------------------------*/
    dialog.Message = function (content, options) {

        var defaults = {
            title: '[Módulo de Seguridad]',
            buttonText: 'Aceptar',
            msgError: "Se ha producido un error",
            msgWarning: "Por Favor Verifique los Datos Ingresados",
            msgSuccess: "La operación se ha realizado correctamente",
            callback: null,
            contentUrl: null,
            extraButtons: null
        }
        ...
        if (typeof content == "object") {
            switch (content.Estado) {
                case 0:
                    var icon = '<i class="fa fa-info-circle" style="color:#04c5ec;padding-right: 5px;"></i>';
                    content = returnMessage(values.msgWarning, content.Mensaje);
                    values.title = icon + values.title;
                    break;
                case 1:
                    var icon = '<i class="fa fa-check-square-o" style="color:#559c56;padding-right: 5px;"></i>';
                    content = returnMessage(values.msgSuccess, content.Mensaje);
                    values.title = icon + values.title;
                    break;
                case 2:
                    var icon = '<i class="fa fa-exclamation-triangle" style="color:#f1605e;padding-right: 5px;"></i>';
                    content = returnMessage(values.msgError, content.Mensaje);
                    values.title = icon + values.title;
                    break;
            }
        }
        ...
        function returnMessage(msgDefault, msg) {
            if (msg)
                return msg;
            return msgDefault;
        }
